...
# Кілтті Streamlit Secrets ішінен қатесіз, қауіпсіз оқу
try:
    GEMINI_API_KEY = st.secrets["GEMINI_API_KEY"]
    genai.configure(api_key=GEMINI_API_KEY)
except Exception as e:
    st.error("Secrets ішінен GEMINI_API_KEY табылмады. Баптауларды тексеріңіз.")
...
# --- STEP 4 ---
    ...
    prompt = f"Сен кәсіптік бағдар берегуші ЖИ-сің. Мына оқушыға Қазақстан нарығына сай ТОП-3 мамандық ұсынып, себебін қысқаша ғылыми негіздеп жаз: {user_profile}. Тек қазақ тілінде жауап бер."
    ...
    with st.spinner("ЖИ мәліметтерді талдауда..."):
        try:
            ...
            st.error("ЖИ-ге қосылу мүмкін болмады. Secrets ішіндегі API кілтін тексеріңіз.")
